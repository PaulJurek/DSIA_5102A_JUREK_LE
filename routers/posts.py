...
async def create_post(post: schemas.Post, user_id : str = Depends(get_user_id), db: Session = Depends(models.get_db)):
...
async def get_post_by_id(
    post_id: str, request: Request, token: int = Depends(verify_autorization_header), db: Session = Depends(models.get_db)
):
